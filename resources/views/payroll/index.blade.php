...
@section('title', 'Payroll - ERP HRIS')
...
@section('content')
    ...
    <div class="sm:flex sm:items-center sm:justify-between mb-8">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">
                Kelola data kompensasi, tunjangan, potongan, dan riwayat slip gaji bulanan karyawan.
            </p>
        </div>

        <div class="mt-4 sm:mt-0 flex items-center gap-3">
            <a href="{{ route('payroll.export') }}"
               class="inline-flex items-center justify-center rounded-xl bg-emerald-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm shadow-emerald-200 hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 transition-all duration-200 hover:-translate-y-0.5 transform">
    ...
    <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 mb-8">
        <div class="bg-white overflow-hidden rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col transition-transform hover:-translate-y-1 duration-300">
            <div class="flex items-center mb-2">
                ...
                <p class="text-sm font-medium text-gray-500">Total Payroll Terbit</p>
            </div>
            <div class="flex items-baseline gap-2">
                <p class="text-3xl font-bold text-gray-900">{{ count($payrolls) }}</p>
                <span class="text-sm text-emerald-600 font-medium">catatan bulan ini</span>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow duration-300">
        ...
        <div class="px-6 py-5 border-b border-gray-100 flex justify-between items-center">
            <h3 class="text-base font-semibold text-gray-900">Riwayat Penggajian</h3>
            <div class="relative">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50/50">
                    <tr>
                        ...
                        <th class="px-3 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-widest">Karyawan</th>
                        <th class="px-3 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-widest">Periode</th>
                        <th class="px-3 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-widest">Gaji Pokok</th>
                        <th class="px-3 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-widest">Tunjangan</th>
                        <th class="px-3 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-widest">Potongan</th>
                        <th class="px-3 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-widest">Total Bersih</th>
                    ...
                    @forelse($payrolls as $payroll)
                        <tr class="hover:bg-indigo-50/40 transition-colors duration-150 group">
                            ...
                            <td class="whitespace-nowrap px-3 py-4">
                                ...
                                <div class="text-xs text-gray-400 mt-0.5">{{ $payroll->employee->job_role }}</div>
                            </td>
                            <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-600">
                                <div class="flex items-center gap-1.5">
                                    ...
                                    Bln {{ $payroll->month }} / {{ $payroll->year }}
                                </div>
                            </td>
                            <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-600">
                                Rp {{ number_format($payroll->basic_salary, 0, ',', '.') }}
                            </td>
                            <td class="whitespace-nowrap px-3 py-4 text-sm text-emerald-600 font-medium">
                                +Rp {{ number_format($payroll->allowances, 0, ',', '.') }}
                            </td>
                            <td class="whitespace-nowrap px-3 py-4 text-sm text-rose-600 font-medium">
                            ...
                            <td class="relative whitespace-nowrap py-4 pl-3 pr-6 text-right text-sm">
                                <div class="flex justify-end gap-3">
                                    ...
                                    <a href="{{ route('payroll.edit', $payroll->id) }}" 
                                       class="text-indigo-600 hover:text-indigo-900 bg-indigo-50 hover:bg-indigo-100 px-3 py-1.5 rounded-lg flex items-center gap-1 transition-colors">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="py-16 text-center">
                                ...
                                <h3 class="text-sm font-semibold text-gray-900">Belum ada Data Payroll</h3>
                                <p class="mt-1 text-sm text-gray-500">Mulai dengan mencatat transaksi slip gaji baru ke dalam sistem.</p>
                                <div class="mt-6">
                                    <a href="{{ route('payroll.create') }}" class="inline-flex items-center rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-700">
                                        Tambah Gaji Baru
                                    </a>
                                </div>
                            </td>
