    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="px-6 py-6">
            <form action="{{ route('jobrole.update', $jobrole->id) }}" method="POST">
                ...
                <div class="mb-5">
                    <label for="department" class="block text-sm font-semibold text-gray-700 mb-1.5">Departemen</label>
                    <select id="department" name="department"
                        ...
                        @foreach(['IT', 'Data', 'Human Resources', 'Product'] as $dept)
                            <option value="{{ $dept }}" {{ old('department', $jobrole->department) == $dept ? 'selected' : '' }}>{{ $dept }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="mb-5">
                    ...
                    <select id="level" name="level"
                        ...
                        @foreach(['Staff', 'Senior', 'Manager'] as $lvl)
                            <option value="{{ $lvl }}" {{ old('level', $jobrole->level) == $lvl ? 'selected' : '' }}>{{ $lvl }}</option>
                        @endforeach
                    </select>
                </div>
