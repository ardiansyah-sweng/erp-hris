    ...
    <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 mb-8">
        <div class="bg-white overflow-hidden rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col transition-transform hover:-translate-y-1 duration-300">
            ...
            <div class="flex items-baseline gap-2">
                <p class="text-3xl font-bold text-gray-900">{{ $jobroles->count() }}</p>
                <span class="text-sm text-emerald-600 font-medium">role terdaftar</span>
            </div>
        </div>
    </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50/50">
                    <tr>
                        ...
                        <th class="px-3 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-widest">Dibuat</th>
                        <th class="px-3 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-widest">Diperbarui</th>
                ...
                <tbody class="divide-y divide-gray-100 bg-white">
                    @forelse($jobroles as $role)
                        <tr class="hover:bg-indigo-50/40 transition-colors duration-150 group">
                            ...
                            <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-600">
                                {{ $role->updated_at ? $role->updated_at->format('d M Y') : '-' }}
                            </td>
                            <td class="relative whitespace-nowrap py-4 pl-3 pr-6 text-right text-sm">
                                ...
                                <div class="flex justify-end gap-3">
                                    ...
                                    <form action="{{ route('jobrole.destroy', $role->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus job role ini?')" class="inline">
                                        @csrf
                                        @method('DELETE')
